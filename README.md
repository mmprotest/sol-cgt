...
4. **Provide your API keys (required for fetch, optional for pricing)**
   ...
   Open `.env` in a text editor and paste your [Helius](https://www.helius.dev/) API key (`HELIUS_API_KEY`, required for `fetch`). A Jupiter API key (`JUP_API_KEY`) is only used for token metadata search. If a non-SOL token price cannot be sourced, it is flagged as a warning and left unpriced instead of crashing.
